...
<head>
  ...
  <style>
    ...
    .badge-PENDING_SPV { color: #ca8a04; border-color: #fde68a; background: #fffbeb; }
    .badge-PENDING_MANAGER { color: #1d4ed8; border-color: #bfdbfe; background: #eff6ff; }
...
<div class="toolbar">
  <div class="toolbar-left">
    ...
    <div>
      ...
      <div class="toolbar-sub">{{ $klien?->nama_klien }} &bull; Ref: {{ $invoice?->no_invoice }}</div>
    </div>
  </div>
</div>
@endif
...
@php
    ...
    // Identitas perusahaan: sama seperti invoice-print (PT → karyawan.perusahaan, else invoice.perusahaan)
    if ($klien && $klien->tipe_klien === 'PT') {
        $namaPerusahaan = $invoice?->karyawan?->perusahaan?->nama_perusahaan
            ?? $invoice?->perusahaan?->nama_perusahaan
            ?? '';
    } else {
        $namaPerusahaan = $invoice?->perusahaan?->nama_perusahaan ?? '';
    }

    $tanggalDokumen = $koreksi->manager_actioned_at ?? $koreksi->submitted_at;
...
<div class="print-container">
        ...
        <td class="info-col">
          <table class="dl-table">
            <tr>
              ...
              <td class="dl-val"><strong style="color:#b71c1c;">{{ $klien?->nama_klien ?? '-' }}</strong></td>
            </tr>
            <tr>
  ...
  <div class="nilai-box">
    <div class="nilai-header {{ $isCn ? 'nilai-header-cn' : 'nilai-header-dn' }}">
      {{ $isCn ? 'Nilai Pengurangan Tagihan' : 'Nilai Penambahan Tagihan' }}
    </div>
    <div class="nilai-body">
      ...
      <div class="nilai-caption">
        {{ $isCn ? 'Pengurangan atas tagihan invoice' : 'Penambahan atas tagihan invoice' }}
        {{ $invoice?->no_invoice ?? '' }}
      </div>
    </div>
  </div>
