...
class ImportInvoiceJob implements ShouldQueue
{
    ...
    public function handle(InvoiceImportService $service): void
    {
        ...
        if (!$batch) {
            Log::warning('ImportInvoiceJob: batch tidak ditemukan', ['id' => $this->batchId]);
            return;
        }

        $user = User::with('karyawan')->find($batch->user_id);
        if (!$user || !$user->karyawan) {
            $batch->update(['status' => 'failed', 'message' => 'User pembuat import tidak valid / tidak terhubung dengan data karyawan.']);
            return;
        }

        // Set authenticated user agar auth()->id()/auth()->user() & BlameableTrait
        // berfungsi sama seperti pada request sinkron sebelumnya.
        Auth::setUser($user);

        try {
            $service->process($batch);
        } catch (Throwable $e) {
            Log::error('ImportInvoiceJob: import gagal', ['batch_id' => $batch->id, 'error' => $e->getMessage()]);
            $batch->update([
                'status'  => 'failed',
    ...
    private function cleanupFile(?InvoiceImportBatch $batch): void
    {
