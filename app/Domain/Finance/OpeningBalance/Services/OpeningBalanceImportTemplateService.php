...
class OpeningBalanceImportTemplateService
{
    ...
    private function buildInstructionSheet(Worksheet $sheet): void
    {
        ...
        $steps = [
            ...
            '4. tipe_klien menerima PT atau B2B (sinonim — klien konsolidasi head office, nama_klien WAJIB dan dicari case-insensitive, DITOLAK jika cocok dengan lebih dari 1 klien PT aktif; kode_resto OPSIONAL & bebas isi di tiap baris kalau invoice historisnya per resto, tidak memengaruhi resolusi klien) maupun RESTO atau B2C (sinonim — klien per outlet, kode_resto WAJIB diisi & dicocokkan ketat ke MASTER DATA; nama_klien untuk RESTO/B2C bersifat OPSIONAL — boleh dikosongkan, resolve murni via kode_resto).',
        ...
        $this->buildColumnDescriptionBlock($sheet, $row, 'DESKRIPSI KOLOM — Sheet "Data Opening Balance"', 'FF2E7D32', 'FF66BB6A', [
            ['nama_klien',            'Nama Client AR aktif. Untuk tipe_klien PT/B2B: WAJIB di baris pertama grup, dicocokkan case-insensitive, DITOLAK jika ambigu (cocok >1 klien PT). Untuk RESTO/B2C: OPSIONAL — boleh dikosongkan, resolve murni via kode_resto (tidak dicocokkan ke MASTER DATA sama sekali).', 'PT/B2B: baris pertama grup', 'Nama Klien Contoh'],
