...
namespace App\Domain\Finance\KlienAr\Jobs;

use App\Domain\Finance\KlienAr\Services\KlienArImportService;
...
class ImportKlienArJob implements ShouldQueue
{
    ...
    public function handle(KlienArImportService $service): void
    {
        $batch = KlienArImportBatch::find($this->batchId);
        if (!$batch) {
            Log::warning('ImportKlienArJob: batch tidak ditemukan', ['id' => $this->batchId]);
            return;
        }
        ...
        try {
            ...
            Log::error('ImportKlienArJob: import gagal', ['batch_id' => $batch->id, 'error' => $e->getMessage()]);
            $batch->update([
                'status'  => 'failed',
    ...
    private function cleanupFile(?KlienArImportBatch $batch): void
    {
