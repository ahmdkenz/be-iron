...
namespace App\Domain\Master\Resto\Jobs;

use App\Domain\Master\Resto\Services\RestoImportService;
...
class ImportRestoJob implements ShouldQueue
{
    ...
    public function handle(RestoImportService $service): void
    {
        $batch = RestoImportBatch::find($this->batchId);
        if (!$batch) {
            Log::warning('ImportRestoJob: batch tidak ditemukan', ['id' => $this->batchId]);
            return;
        }
        ...
        try {
            ...
            Log::error('ImportRestoJob: import gagal', ['batch_id' => $batch->id, 'error' => $e->getMessage()]);
            $batch->update([
                'status'  => 'failed',
    ...
    private function cleanupFile(?RestoImportBatch $batch): void
    {
