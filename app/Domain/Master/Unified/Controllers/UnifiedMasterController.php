...
class UnifiedMasterController extends Controller
{
    ...
    private function buildMasterDataSheet(Worksheet $sheet): void
    {
        ...
        $sheet->setCellValue('A2', 'Satu baris = 1 outlet (Investor + Resto + Client AR). Kolom bertanda (*) wajib diisi — sebagian bersifat kondisional: nama_entitas & pic_ar wajib jika tipe_klien=PT/B2B; nama_pic wajib jika tipe_klien=RESTO/B2C. nama_investor boleh dikosongkan untuk RESTO/B2C — Client AR outlet tsb otomatis pakai nama fallback "kode_resto (nama_cabang)" (Investor tidak dibuat). tipe_klien boleh diisi RESTO/B2C atau PT/B2B (RESTO & B2C sama artinya, begitu juga PT & B2B). Kolom nama_pic & pic_ar boleh diisi NAMA karyawan, NIK/kode karyawan (boleh alfanumerik, mis. "FL0401780"), atau kombinasi "Nama (NIK)"/"Nama - NIK"/"Nama / NIK" (mis. "Gusti Erwanda - FL0401780"). NPWP & kontak Client AR otomatis dari Investor baris ini (npwp → no_npwp, no_hp → no_wa) untuk semua tipe. Hanya role ADMIN/MANAGER/SUPERVISOR. Lihat sheet "Petunjuk Pengisian".');
    ...
    private function buildInstructionSheet(Worksheet $sheet): void
    {
        ...
        $steps = [
            ...
            '5. Kolom nama_pic dan pic_ar boleh diisi NAMA KARYAWAN, NIK/kode karyawan (boleh alfanumerik, mis. "FL0401780"), atau kombinasi keduanya ("Andi Wijaya (3273010101900001)", "Gusti Erwanda - FL0401780", atau "Andi Wijaya / 3273010101900001") — sistem mencari otomatis. Untuk format kombinasi, NIK dipakai sebagai identitas utama: jika NIK ditemukan tapi nama di Excel berbeda dari Master Karyawan, nama karyawan akan DIPERBARUI mengikuti Excel (NIK tidak pernah berubah, dan karyawan baru tidak akan dibuat otomatis). Nama Client AR otomatis: RESTO = nama_investor (jika kosong, fallback ke "kode_resto (nama_cabang)" tanpa membuat Investor), PT = nama_entitas.',
        ...
        $masterDataCols = [
            ...
            ['nama_pic',        'Nama PIC Resto — boleh diisi NAMA karyawan, NIK/kode karyawan (boleh alfanumerik), atau kombinasi "Nama (NIK)"/"Nama - NIK"/"Nama / NIK" (lookup). Kombinasi memakai NIK sebagai identitas utama — kalau nama di Excel beda dari Master Karyawan, nama akan ditimpa mengikuti Excel. WAJIB jika tipe_klien=RESTO/B2C (sekaligus jadi PIC AR Client). Jika kosong, pic_ar dipakai sebagai pengganti (berlaku untuk PT/B2B maupun RESTO/B2C). Jika nama_pic & pic_ar sama-sama diisi tapi menunjuk karyawan berbeda, baris ditolak.', 'Ya (RESTO/B2C)', 'Gusti Erwanda - FL0401780'],
            ...
            ['pic_ar',          'Nama karyawan AR — boleh diisi NAMA karyawan, NIK/kode karyawan (boleh alfanumerik), atau kombinasi "Nama (NIK)"/"Nama - NIK"/"Nama / NIK" (lookup). Kombinasi memakai NIK sebagai identitas utama — kalau nama di Excel beda dari Master Karyawan, nama akan ditimpa mengikuti Excel. WAJIB jika tipe_klien=PT/B2B (jadi PIC AR Client). Untuk tipe_klien=RESTO/B2C bersifat opsional (hanya dipakai sebagai pengganti jika nama_pic kosong). NPWP & kontak Client AR otomatis dari Investor baris ini (npwp → no_npwp, no_hp → no_wa) untuk semua tipe.', 'Ya (PT/B2B)', 'Siti Rahayu / 3273010101900002'],
